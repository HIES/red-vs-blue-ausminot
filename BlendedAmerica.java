        ...
        for (String reg : outer.keySet())
        {
            ...
            while (inputObj.hasNextLine())
            {
                ...
                if (outer.get(reg).containsKey(tempV[0]))
                {
                    ...
                    double voteTotal = rVotes[0] + rVotes[1] + rVotes[2];
                    int r = (int)(255*((1.0*rVotes[0])/voteTotal));
                    int b = (int)(255*((1.0*rVotes[1])/voteTotal));
                    int g = (int)(255*((1.0*rVotes[2])/voteTotal));
                    
                    Color blended = new Color(r, g, b);
                    for (int i = 0; i < temp.size(); ++i)
                    {
                        temp.get(i).changeColor(blended);
                    }
                }
            }
